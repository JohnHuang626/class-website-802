...
const INITIAL_PHOTOS = [
  ...
  {
    id: 5,
    url: 'https://images.unsplash.com/photo-1427504494785-319ce8372379?auto=format&fit=crop&q=80&w=800',
    title: '班級讀書會',
    date: '2025-03-15',
  },
  {
...
export default function App() {
  // 【終極備案】強制載入 Tailwind CSS 樣式引擎
  // 如果終端機或設定檔設定失敗，這段程式碼會確保畫面依然有顏色與排版
  useEffect(() => {
  ...
  const submitLogin = () => {
    if (adminPwd === 'admin') {
      ...
      setLoginError('密碼錯誤！請輸入預設密碼 admin');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-gray-800 font-sans selection:bg-blue-200 flex flex-col">
      ...
      <main className="pt-24 pb-20 md:pb-12 flex-grow">
        ...
        {activeTab === 'photos' && (
          <PhotosView isAdmin={isAdmin} ConfirmModal={ConfirmModal} />
        )}
        {activeTab === 'info' && <ClassInfoView isAdmin={isAdmin} />}
        {activeTab === 'resources' && (
          <ResourcesView isAdmin={isAdmin} ConfirmModal={ConfirmModal} />
        )}
      ...
      <footer className="bg-white border-t border-gray-200 py-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 flex flex-col items-center justify-center">
          <p className="text-gray-500 text-sm">
            © 2026 {CLASS_INFO.name}. All rights reserved.
          </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {[
            ...
            >
              <div
                className={`w-14 h-14 ${item.color} rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
              >
                {item.icon}
              </div>
...
function ClassInfoView({ isAdmin }) {
  ...
  const handleScheduleChange = (id, field, value) => {
    setSchedule(
      schedule.map((row) => (row.id === id ? { ...row, [field]: value } : row))
    );
  };
                  ...
                  <React.Fragment key={row.id}>
                    {idx === 4 && (
                      <tr className="bg-blue-50/50">
                        <td
                          colSpan="6"
                          className="py-3 text-sm font-medium text-blue-600"
                        >
                          午休時間 12:00 - 13:30
                        </td>
                      </tr>
                    ...
                    {isEditing ? (
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full text-sm text-blue-600 border rounded p-1"
                          placeholder="姓名"
                        />
                        <input
                          value={student.desc}
                          ...
                          className="w-full text-xs text-gray-500 border rounded p-1"
                          placeholder="說明"
                        />
                      </div>
                    ) : (
...
function ResourcesView({ isAdmin, ConfirmModal }) {
  ...
  const confirmAddVideo = () => {
    if (newVideoTitle.trim()) {
      setVideos([
        ...videos,
        { id: Date.now(), title: newVideoTitle, duration: '00:00', views: 0 },
      ]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative">
            ...
            {isAdmin && (
              <button
                ...
                className="absolute top-6 right-6 flex items-center gap-1 text-sm bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg hover:bg-blue-100 font-medium"
              >
                <Plus size={16} /> 新增影片
              </button>
            )}
            ...
            <div className="space-y-4">
              {videos.map((video) => (
                <div
                  key={video.id}
                  className="flex gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer group border border-transparent hover:border-gray-100 relative"
                >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative">
            ...
            {isAdmin && (
              <button className="absolute top-6 right-6 text-blue-500 hover:text-blue-700">
                <Plus size={16} />
              </button>
            )}
            <div className="space-y-2">
              {[
                ...
                <button
                  key={i}
                  className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-blue-50 text-left group transition-colors"
                >
                  <span className="text-sm font-medium text-gray-700 group-hover:text-blue-700 truncate pr-4">
                    {file.title}
                  </span>
                  <span className="text-[10px] font-bold px-2 py-1 bg-gray-100 text-gray-500 rounded group-hover:bg-blue-200 group-hover:text-blue-800">
                    {file.ext}
                  </span>
                </button>
              ))}
...
function DiscussionView({ user, isAdmin, ConfirmModal }) {
  ...
  const handleSendMessage = (e) => {
    ...
    const displayName =
      customName.trim() ||
      (role === 'teacher' ? 'John 老師' : role === 'parent' ? '家長' : '學生');
              ...
              return (
                <div
                  ...
                  className={`flex flex-col max-w-[80%] ${
                    isTeacher ? 'items-start' : 'items-end self-end ml-auto'
                  } group`}
                >
